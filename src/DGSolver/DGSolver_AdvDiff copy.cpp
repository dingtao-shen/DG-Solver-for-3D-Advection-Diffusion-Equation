#include "DGSolver/DGSolver_AdvDiff.hpp"
#include <omp.h>
#include <iostream>
#include <stdexcept>
#include <fstream>
...
namespace DGSolver {
    ...
    template<>
    DGSolver_AdvDiff<3>::DGSolver_AdvDiff(alignedVec<MatrixXd>& MassMat, alignedVec<alignedVec<MatrixXd>>& StfMat, 
                                            alignedVec<alignedVec<MatrixXd>>& MassOnFace, alignedVec<alignedVec<MatrixXd>>& DivDivMat,
                                            ...
                                            std::vector<int>& comp_odr)
    {
        ...
        for(int l = 0; l < CC.N_MESH_CELL; l++){
            ...
            for(int i = 0; i < 3; i++){
                ...
                // Coeff = Coeff.array() + K * DivDivMat[CellID][i].array();
            }
            ...
            for(int il = 0; il < 4; il++){
                ...
                if(BCTAG == 0){
                    ...
                    // for(int i = 0; i < 3; i++){
                    //     Coeff = Coeff.array() - K * StiffFaceMat[CellID][il][i].array() * Cells[CellID]->getOutwardNormVec()[il](i);
                    //     Coeff = Coeff.array() + 0.5 * K * StiffFluxMat[CellID][il][i].array() * Cells[CellID]->getOutwardNormVec()[il](i);
                    //     Coeff = Coeff.array() - 0.5 * K * StiffFluxMat[NBR][il][i].array() * Cells[CellID]->getOutwardNormVec()[il](i);
                    // }

                    // Coeff = Coeff.array() + sigma * MassOnFace[CellID][il].array();
                    // Coeff = Coeff.array() - sigma * FluxInt[CellID][il].array();
        ...
        cout << "DGSolver_AdvDiff initialized" << endl;
    }

    template<>
    DGSolver_AdvDiff<3>::~DGSolver_AdvDiff() {}

    template<>
    void DGSolver_AdvDiff<3>::solve(alignedVec<std::shared_ptr<Element::FaceDim<3, 3>>>& Faces, 
                                    alignedVec<std::shared_ptr<Element::CellDim<3, 4>>>& Cells, 
                                    std::vector<int>& comp_odr,
                                    alignedVec<MatrixXd>& MassMat, alignedVec<alignedVec<MatrixXd>>& MassOnFace, alignedVec<alignedVec<MatrixXd>>& FluxInt,
                                    alignedVec<alignedVec<alignedVec<Eigen::MatrixXd>>>& StiffFaceMat,
                                    alignedVec<alignedVec<alignedVec<Eigen::MatrixXd>>>& StiffFluxMat,
                                    alignedVec<alignedVec<VectorXd>>& IntFaceMat,
                                    alignedVec<VectorXd>& SolutionPrev) 
    {
        omp_set_num_threads(omp_get_max_threads());
        // #pragma omp parallel for
        for(int l = 0; l < CC.N_MESH_CELL; l++){
            ...
            VectorXd A_src = 1.0 / dt * MassMat[CellID] * SolutionPrev[CellID];
            ...
            for(int il = 0; il < 4; il++){
                ...
                if(BCTAG == 0){
                    ...
                    A_src.noalias() -= 0.5 * (f - abs(f)) * (FluxInt[CellID][il] * SolutionPrev[NBR]);
                    // for(int i = 0; i < 3; i++){
                    //     A_src.noalias() += K * 0.5 * StiffFluxMat[NBR][il][i] * Cells[CellID]->getOutwardNormVec()[il](i) * SolutionPrev[NBR];
                    //     A_src.noalias() -= K * StiffFaceMat[NBR][il][i] * Cells[CellID]->getOutwardNormVec()[il](i) * SolutionPrev[NBR];
                    //     A_src.noalias() -= K * 0.5 * StiffFluxMat[CellID][il][i] * Cells[CellID]->getOutwardNormVec()[il](i) * SolutionPrev[NBR];
                    // }

                    // A_src.noalias() += sigma * FluxInt[NBR][il] * SolutionPrev[NBR];
                    // A_src.noalias() -= sigma * MassOnFace[NBR][il] * SolutionPrev[NBR];
                }
                else if(CC.BOUNDARY_COND[BCTAG].first == 1){
                    double f = 0.0;
                    for(int i = 0; i < 3; i++){
                    ...
                    // for(int d = 0; d < 3; d++){
                    //     A_src.noalias() -= K * 0.5 * (f - abs(f)) * StiffFluxMat[CellID][il][d].col(0) * Cells[CellID]->getOutwardNormVec()[il](d);
                    // }

                    // A_src.noalias() += sigma * FluxInt[CellID][il].col(0);
                }
            }
    ...
    template<>
    alignedVec<VectorXd> DGSolver_AdvDiff<3>::getSolution() const {
        return Solution;
    }

    template<>
    void DGSolver_AdvDiff<3>::save(alignedVec<std::shared_ptr<Element::CellDim<3, 4>>>& Cells, std::string results_dir, int fix_crd_idx, double crd) {
        ...
        ofstream write_output(results_dir + "/AdvDiff_solution.txt");
